...
// Get references to all elements with class 'colorButton'
const colorButtons = document.querySelectorAll(".colorButton");
const colorButtons2 = document.querySelectorAll(".colorButton2");
// Loop through each element and add event listener for click event
...
colorButtons2.forEach(function (colorButton2) {
  colorButton2.addEventListener("click", function (event) {
    // Prevent the default link behavior
    event.preventDefault();

    // Remove 'clicked' class from all buttons
    colorButtons2.forEach(function (btn) {
      btn.classList.remove("clicked");
    });
...
jQuery(document).ready(function ($) {
  ...
  // custom code
  const countDown = () => {
    const countDate = new Date("March 13, 2024 00:00:00").getTime();
